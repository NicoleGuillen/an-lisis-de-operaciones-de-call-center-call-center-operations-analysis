...
# Gráfico 1: Relación entre AHT y Nivel de Servicio
plt.subplot(1, 2, 1)
sns.scatterplot(data=df, x='AHT_Segundos', y='Service_Level_Decimal', hue='Categoria_AHT', palette='viridis')
plt.title('Impacto del AHT en el Nivel de Servicio')
plt.xlabel('AHT (Segundos)')
plt.ylabel('Nivel de Servicio (%)')




# Gráfico 2: Distribución del AHT según el Tráfico de llamadas
plt.subplot(1, 2, 2)
sns.boxplot(data=df, x='Segmento_Trafico', y='AHT_Segundos', palette='Set2')
plt.title('Distribución del AHT por Nivel de Tráfico')
plt.xlabel('Volumen de Llamadas Entrantes')
...
#  Configurar el estilo estético del gráfico
sns.set_theme(style="whitegrid")
plt.figure(figsize=(10, 6))


#  Crear el gráfico de dispersión con línea de regresión
# 'regplot' dibuja los puntos y calcula automáticamente la línea de tendencia
sns.regplot(
    data=df,
    x="Answer_Speed_Segundos",
    y="Abandon_Rate_Porcentaje",
    scatter_kws={"alpha": 0.6, "color": "#1f77b4"},  # Transparencia en puntos
...
plt.title(
    "¿Influye la Velocidad de Respuesta en el Abandono?",
    fontsize=14,
    pad=15,
    weight="bold",
)
plt.xlabel("Velocidad Promedio de Respuesta (Segundos)", fontsize=12)
plt.ylabel("Tasa de Abandono (%)", fontsize=12)


# Limitamos los ejes para que el gráfico se vea limpio si hay algún outlier
plt.ylim(-5, 105)
